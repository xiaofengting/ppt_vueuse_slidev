...
import { whenever, useCounter } from '@vueuse/core'
import { useMagicKeys, useBattery } from '@vueuse/core'
...
const { q_e } = useMagicKeys()
whenever(q_e, () => { inc() })

watch(count, () => {
  const optionIndex = (count.value + 1) % CircleOptions.length;
  myChart.setOption(CircleOptions[optionIndex]);
})

const { charging } = useBattery()

watch(charging, () => {
  if ($slidev.nav.currentPage === 65)
    inc()
})
